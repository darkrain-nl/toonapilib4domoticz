...
class DevicePowerDailyCost(Device):
    domoticz_device_type = 243
    domoticz_subtype = 31

    def __init__(self, plugin_devices, toon):
        super().__init__(config.STR_UNIT_POWER_DAILY_COST,
                         config.STD_UNIT_POWER_DAILY_COST,
                         plugin_devices,
                         toon)
    ...
    def create(self):
        if not self.exists:
            try:
                Domoticz.Log("Creating daily cost power device " + self.name)
    ...
    def update(self):
        ...
        try:
            str_value = str(self.toon.power.daily_cost)

            if str_value != self.previous_value:
                if config.debug:
                    Domoticz.Log("Update daily cost power: " + str_value)
